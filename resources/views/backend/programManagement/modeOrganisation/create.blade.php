...
<div class="card">
    <div class="card-body p-4">
        ...
        <form action="{{route("modeOrganisations.store")}}" method="post">
        @csrf
        <div class="row">
            <div class="col-lg-12">
            <div class="border border-3 p-4 rounded">
                    <div class="mb-3">
                        <label for="inputProductTitle" class="form-label h6">Code du mode organisation</label>
                        <input type="text" name="code" value="{{old("code")}}" class="form-control @error('code') is-invalid @enderror" id="inputProductTitle" placeholder="Saisissez le code du mode organisation">
                        @error('code')
                            <span class="text-danger"> {{ $message }}</span>
                        @enderror
                    </div>
                     <div class="mb-3">
                        <label for="inputProductTitle" class="form-label h6">Libelle du mode organisation</label>
                        <input type="text" name="libelle" class="form-control @error('libelle') is-invalid @enderror" value="{{old("libelle")}}" id="libelleId" placeholder="Saisissez du mode organisation ">
                        @error('libelle')
                            <span class="text-danger"> {{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <select class="form-select select2  @error('methode_costing_id') is-invalid @enderror" id="methode_costing_id" name="methode_costing_id">
                            <option value="">-- Sélectionnez une methode organisation --</option>
                            @if (count($datas)>0)
                                @foreach ($datas as $d)
                                <option value="{{$d->id}}" {{old("methode_costing_id")==$d->id ? 'selected' : ''}}>{{$d->code}} - {{$d->libelle}}</option>
                                @endforeach
                            @endif
                        </select>
