        ...
        <div class="row">
            ...
            <div class="border border-3 p-4 rounded">
                    <div class="mb-3">
                        ...
                        <input type="text" name="code" value="{{old("code")}}" class="form-control @error('code') is-invalid @enderror" id="inputProductTitle" placeholder="Saisissez le code du chapiter ">
                        @error('code')
                     ...
                     <div class="mb-3">
                        ...
                        <input type="text" name="libelle" class="form-control @error('libelle') is-invalid @enderror" value="{{old("libelle")}}" id="inputProductTitle" placeholder="Saisissez le chapiter ">
                        @error('libelle')
                            <span class="text-danger"> {{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <select class="form-select select2  @error('section_id') is-invalid @enderror" id="ministereSelect" name="section_id">
                            ...
                            @if (count($datas)>0)
                                @foreach ($datas as $m)
                                    <option value="{{$m->id}}" {{old("section_id")==$m->id ? 'selected' : ''}}>{{$m->code}} - {{$m->libelle}}</option>
                                @endforeach
                            @endif
                        </select>
                        @error('section_id')
