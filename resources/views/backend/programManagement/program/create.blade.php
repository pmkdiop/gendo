    ...
    <div class="card-body p-4">
        <div class="d-lg-flex align-items-center mb-4 gap-3">
            <div class="position-relative">
                <h6 class="mb-0 text-uppercase"> <i class="bx bxs-plus-circle text-primary"></i> Creation d'un nouveau programme</h6>
            </div>
        ...
        <div class="row">
            ...
            <div class="border border-3 p-4 rounded">
                <div class="mb-3">
                        ...
                        <input type="text" name="code" value="{{old('code')}}" class="form-control @error('code') is-invalid @enderror" id="inputProductTitle" placeholder="Saisissez le code de programme ">
                        @error('code')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="libelle" value="{{old('libelle')}}" class="form-control @error('libelle') is-invalid @enderror" id="inputProductTitle" placeholder="Saisissez le libelle du programme ">
                        @error('libelle')
                            <span class="text-danger"> {{ $message }}</span>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror" name="description" id="inputProductDescription" rows="7" placeholder="Saisissez la description du programme">{{old('description')}}</textarea>
                         @error('description')
                            <span class="text-danger"> {{ $message }}</span>
                        @enderror
                    </div>
            ...
            <div class="border border-3 p-4 rounded">
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="dateDebut" value="{{old("dateDebut")}}" class="form-control  @error('dateDebut') is-invalid @enderror" id="inputPrice" placeholder="Ex: 6123">
                    @error('dateDebut')
                        <span class="text-danger"> {{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="dateFin" value="{{old("dateFin")}}" class="form-control  @error('dateFin') is-invalid @enderror" id="inputPrice" placeholder="Ex: 6123">
                    @error('dateFin')
                        <span class="text-danger"> {{ $message }}</span>
                    @enderror
                </div>
                    ...
                    <select class="form-select select2  @error('ministere_id') is-invalid @enderror" id="ministereSelect" name="ministere_id" onchange="fetchSections()">
                            ...
                            @if (count($ministeres)>0)
                                @foreach ($ministeres as $m)
                                    <option value="{{$m->id}}">{{$m->code}} - {{$m->nom}}</option>
                                @endforeach
                            @endif
                        </select>
                        @error('ministere_id')
