...
<div class="card">
    ...
    <div class="d-lg-flex align-items-center mb-4 gap-3">
        <div class="position-relative">
            <h6 class="mb-0 text-uppercase"> <i class="bx bxs-plus-circle text-primary"></i> La Liste des activités</h6>
        </div>
        <div class="ms-auto"><a href="{{route("activities.create")}}" title="Creer une nouvelle section" class="btn btn-outline-primary radius-30 mt-2 mt-lg-0"><i class="bx bxs-plus-square"></i>Ajouter</a></div>
    </div> <hr>
            ...
            <table id="example2" class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <th>Code</th>
                        <th>Libelle</th>
                        <th>Action de l'activité</th>
                        <th>Chapiter de l'activité</th>
                        <th>Mode organisation de l'activité</th>
                ...
                <tbody>
                    ...
                    @foreach ($datas as $d)
                        <tr>
                            ...
                            <td>{{$d->code}}</td>
                            <td>{{$d->libelle}}</td>
                            <td>{{$d->action->code}} - {{$d->action->libelle}}</td>
                            <td>{{$d->chapter->code}} - {{$d->chapter->libelle}}</td>
                            <td>{{$d->modeOrganisation->code}} - {{$d->modeOrganisation->libelle}}</td>
                             <td>
                                <div class="d-flex order-actions">
                                    ...
                                    <a class="ms-1" href="{{ route('activities.edit', $d->id) }}">
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <form action="{{ route('activities.destroy', $d->id) }}"
                                                    method="post">
                                                    @csrf
                                                    @method('DELETE')
