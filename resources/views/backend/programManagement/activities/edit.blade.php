    ...
    <div class="card-body p-4">
        ...
        <div class="row">
            ...
            <div class="border border-3 p-4 rounded">
                    <div class="mb-3">
                        <label for="inputProductTitle" class="form-label h6">Code de l'activité</label>
                        <input type="text" name="code" value="{{$data->code}}" class="form-control @error('code') is-invalid @enderror" id="codeId" placeholder="Saisissez le code de l'activité ">
                        @error('code')
                            <span class="text-danger"> {{ $message }}</span>
                        @enderror
                    </div>
                     <div class="mb-3">
                        <label for="inputProductTitle" class="form-label h6">Libelle de l'activité</label>
                        <input type="text" name="libelle" class="form-control @error('libelle') is-invalid @enderror" value="{{$data->libelle}}" id="libelleId" placeholder="Saisissez le libelle de l'activité ">
                        @error('libelle')
                            <span class="text-danger"> {{ $message }}</span>
                        @enderror
                    </div>
                    <div class="mb-3">
                    <label for="inputVendor" class="form-label h6">Action de l'activité</label>
                    <select class="form-select select2  @error('action_id') is-invalid @enderror" id="action_id" name="action_id">
                            ...
                            @if (count($actions)>0)
                                @foreach ($actions as $m)
                                    <option value="{{$m->id}}" {{$data->action_id==$m->id ? 'selected' : ''}}>{{$m->code}} - {{$m->libelle}}</option>
                                @endforeach
                            @endif
                        </select>
                        @error('action_id')
                    ...
                    <select class="form-select select2  @error('chapter_id') is-invalid @enderror" id="chapter_id" name="chapter_id">
                            <option value="">-- Sélectionnez un chapiter --</option>
                            @if (count($chapters)>0)
                                @foreach ($chapters as $m)
                                    <option value="{{$m->id}}" {{$data->chapter_id==$m->id ? 'selected' : ''}}>{{$m->code}} - {{$m->libelle}}</option>
                                @endforeach
                            @endif
                        </select>
                        @error('chapter_id')
                    ...
                    <select class="form-select select2  @error('mode_organisation_id') is-invalid @enderror" id="mode_organisation_id" name="mode_organisation_id" >
                            <option value="">-- Sélectionnez un mode organisation de l'activité --</option>
                            @if (count($modeOrgs)>0)
                                @foreach ($modeOrgs as $m)
                                    <option value="{{$m->id}}" {{$data->mode_organisation_id==$m->id ? 'selected' : ''}}>{{$m->code}} - {{$m->libelle}}</option>
                                @endforeach
                            @endif
                        </select>
