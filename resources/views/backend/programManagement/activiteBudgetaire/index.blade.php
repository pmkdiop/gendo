...
<div class="card">
    ...
    <div class="d-lg-flex align-items-center mb-4 gap-3">
        <div class="position-relative">
            <h6 class="mb-0 text-uppercase"> <i class="bx bxs-plus-circle text-primary"></i> Liste des activité Budgetaires</h6>
        </div>
        <div class="ms-auto"><a href="{{route("activitieBudgetaires.create")}}" title="Creer un nouveau programme" class="btn btn-outline-primary radius-30 mt-2 mt-lg-0"><i class="bx bxs-plus-square"></i>Ajouter</a></div>
    </div> <hr>
            ...
            <table id="example2" class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <th>Qté</th>
                        <th>Unité Indicateur</th>
                        <th>Realisation</th>
                        <th>volume</th>
                        <th>Activité</th>
                        <th>Sous activité Budgetaire</th>
                ...
                <tbody>
                    ...
                    @foreach ($datas as $d)
                        <tr>
                            <td>{{$d->produitService}}</td>
                            <td>{{$d->quantite}}</td>
                            <td>{{$d->uniteIndicateur}}</td>
                            <td>{{$d->realisation}}</td>
                            <td>{{$d->volume}}</td>
                            <td> {{ optional($d->activity)->libelle ?? 'N/A' }}</td>
                            <td> {{ optional($d->parent)->produitService ?? 'N/A' }}</td>
                             <td>
                                <div class="d-flex order-actions">
                                    ...
                                    <a class="ms-1" href="{{ route('activitieBudgetaires.edit', $d->id) }}">
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <form action="{{ route('activitieBudgetaires.destroy', $d->id) }}"
                                                    method="post">
                                                    @csrf
                                                    @method('DELETE')
